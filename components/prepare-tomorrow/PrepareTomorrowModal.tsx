...
export function PrepareTomorrowModal({
  ...
  const [selected, setSelected] = useState<Set<string>>(
    () => new Set(initialCandidates.map((t) => t.id)),
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const result = await prepareTomorrow(todayDateStr, [...selected]);
                ...
                {candidates.map((todo) => (
                  <li key={todo.id}>
                    <label
                      ...
                      style={{
                        borderColor: selected.has(todo.id)
                          ? "var(--color-accent)"
                          : "var(--color-rule)",
                        background: selected.has(todo.id)
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span
                          ...
                          style={{ color: "var(--color-ink-muted)", fontFamily: "var(--font-body)" }}
                        >
                          {todo.planned_minutes}分
                          {todo.scheduled_start ? " · 配置済み" : " · 未配置"}
                        </span>
                      </span>
                    </label>
                  </li>
